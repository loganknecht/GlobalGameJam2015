    ...
    public static class fiStorageManager {
        ...
        public static void MigrateStorage() {
            ...
            while (i < fiPrefabManager.Storage.Objects.Count) {
                ...
                // This is a prefab backup, so we don't want to migrate it to scene storage. It
                // should be available regardless of the current scene.
                if (PrefabUtility.GetPrefabType(obj.Target.Target) == PrefabType.Prefab) {
                    ++i;
                }
