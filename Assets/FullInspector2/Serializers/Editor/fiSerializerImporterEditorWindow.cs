...
namespace FullInspector.Internal {
    ...
    [InitializeOnLoad]
    public class fiSerializerImporterEditorWindow : EditorWindow {
        private List<fiSerializationPackage> Packages = new List<fiSerializationPackage>() {
            new fiSerializationPackage {
                ...
                PackagePath = fiUtility.CombinePaths(fiSettings.RootDirectory, "Serializers/FullSerializerPackage.unitypackage"),
                PackageDirectory = fiUtility.CombinePaths(fiSettings.RootDirectory, "Serializers/FullSerializer"),
        ...
        static fiSerializerImporterEditorWindow() {
            if (fiSerializerProxy.HasDefault == false) {
                EditorApplication.delayCall += ShowWindow;
            }
        }

        [MenuItem("Window/Full Inspector/Developer/Show Serializer Importer")]
        public static void ShowWindow() {
            EditorWindow window = EditorWindow.GetWindow<fiSerializerImporterEditorWindow>();
            ...
            window.title = "Serializer Importer";
        }
        ...
        private static void RepaintOnScriptReload() {
            var window = EditorWindow.FindObjectOfType<fiSerializerImporterEditorWindow>();
            if (window != null) {
                window.Repaint();
            }
        ...
        private static void DisplayOption(fiSerializationPackage package) {
            ...
            if (fiSerializerProxy.HasDefault) {
                isDefault = package.SerializerGuid == fiSerializerProxy.DefaultMetadata.SerializerGuid;
            }

            // show the remove/import button, depending on if the serializer is loaded
            if (fiSerializerProxy.IsLoaded(package.SerializerGuid)) {
            ...
            if (isDefault) {
                ...
                EditorGUI.BeginDisabledGroup(!fiSerializerProxy.IsLoaded(package.SerializerGuid));
        ...
        public void OnGUI() {
            ...
            GUILayout.Label("Full Inspector Serializer Importer", TitleStyle);
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
